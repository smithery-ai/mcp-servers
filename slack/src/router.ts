...
export function mcpAuthRouter(options: AuthRouterOptions): RequestHandler {
  ...
  const authorization_endpoint = "/raw/authorize";
  const token_endpoint = "/raw/token";
  const registration_endpoint = options.provider.clientsStore.registerClient ? "/raw/register" : undefined;
  const revocation_endpoint = options.provider.revokeToken ? "/raw/revoke" : undefined;
