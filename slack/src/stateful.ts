...
import { parseAndValidateConfig } from "@smithery/sdk/shared/config.js"
...
import {
	type SessionStore,
	createLRUStore,
} from "@smithery/sdk/server/session.js"
		...
		if (sessionId && sessionStore.get(sessionId)) {
			...
			try {
				const server = createMcpServer({
					...
					config: config as T,
					// NOTE: Injected variable.
					auth: (req as any).auth as AuthInfo,
				})
