...
export function requireBearerAuth({ provider, requiredScopes = [] }: BearerAuthMiddlewareOptions): RequestHandler {
  return async (req, res, next) => {
    try {
      console.log("in requireBearerAuth middleware");
      const authHeader = req.headers.authorization;

      if (!authHeader) {
        console.log("no auth header in requireBearerAuth middleware");
      ...
      if (type.toLowerCase() !== 'bearer' || !token) {
        console.log("invalid auth header format in requireBearerAuth middleware");
      ...
      if (requiredScopes.length > 0) {
        ...
        if (!hasAllScopes) {
          console.log("does not have all scopes in requireBearerAuth middleware", hasAllScopes);
      ...
      if (!!authInfo.expiresAt && authInfo.expiresAt < Date.now() / 1000) {
        console.log("token has expired in requireBearerAuth middleware", authInfo.expiresAt);
