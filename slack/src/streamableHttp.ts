...
import { isInitializeRequest, isJSONRPCError, isJSONRPCRequest, isJSONRPCResponse, JSONRPCMessage, JSONRPCMessageSchema, RequestId } from "@modelcontextprotocol/sdk/types.js";
...
export class StreamableHTTPServerTransport implements Transport {
  ...
  async handleRequest(req: IncomingMessage & { auth?: AuthInfo }, res: ServerResponse, parsedBody?: unknown): Promise<void> {
    // log auth info if it exists
    if (req.auth) {
      console.log("req.auth in handleRequest", req.auth);
    }
    if (req.method === "POST") {
  ...
  private async handlePostRequest(req: IncomingMessage & { auth?: AuthInfo }, res: ServerResponse, parsedBody?: unknown): Promise<void> {
    try {
      ...
      // log auth info if it exists
      if (req.auth) {
        console.log("req.auth in handlePostRequest", req.auth);
      }
      ...
      // log auth info if it exists
      if (authInfo) {
        console.log("authInfo in handlePostRequest", authInfo);
      }
